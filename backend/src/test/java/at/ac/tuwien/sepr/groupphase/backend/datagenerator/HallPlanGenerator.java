...
public class HallPlanGenerator {
    ...
    private String getHallPlanOpenAirImage() {
        String content = "error while loading hallplan image";
        try {
            content = new String(Files.readAllBytes(Paths.get("backend/src/test/resources/datagen/hallplan/openair.txt")));
        } catch (IOException e) {
            e.printStackTrace();
        }
        return content;
