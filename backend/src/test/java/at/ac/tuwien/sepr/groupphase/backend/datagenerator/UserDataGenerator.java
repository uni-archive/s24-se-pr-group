...
public class UserDataGenerator {
    ...
    private static final ApplicationUser CUSTOMER_USER = new ApplicationUser("user@email.com", "", "Erna", "Erna", "+4367768822344", "zyxwvutsrqponmlkjihgfedcba", 0, false, UserType.CUSTOMER, false, true, new Address());
    ...
    private void generateRandomUsers(Long numberOfUsers) {
        ...
        for (int i = 0; i < numberOfUsers; i++) {
            ...
            Address address = new Address(faker.address().streetAddress(), faker.address().zipCode(), faker.address().city(), faker.address().country());

            addresses.add(address);

            ApplicationUser user = new ApplicationUser(email, "", firstName, lastName, phoneNumber, salt, 0, false, UserType.CUSTOMER, false, true, address);
